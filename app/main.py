...
def deposit(account_id: int, tx: schemas.TransactionBase, db: Session = Depends(get_db)):
    return crud.deposit(db, account_id, tx.amount)

@app.post("/accounts/{account_id}/withdraw")
def withdraw(account_id: int, tx: schemas.TransactionBase, db: Session = Depends(get_db)):
    try:
        return crud.withdraw(db, account_id, tx.amount)
